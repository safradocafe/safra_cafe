...
def converter_para_kg(valor, unidade):
    ...
    if unidade == 'latas': return v * 1.8
    if unidade == 'litros': return v * 0.09
    return v
...
def processar_arquivo_carregado(uploaded_file, tipo='amostral'):
    try:
        ...
        if not uploaded_file.name.lower().endswith('.gpkg'):
            st.error("❌ O arquivo deve ter extensão .gpkg"); return None

        temp_file = f"/tmp/{uploaded_file.name}"
        with open(temp_file, "wb") as f: f.write(uploaded_file.getbuffer())
        gdf = gpd.read_file(temp_file); os.remove(temp_file)
        ...
        if tipo == 'amostral':
            if gdf.empty or not gdf.geom_type.isin(['Polygon', 'MultiPolygon']).any():
                st.error("❌ O arquivo da área amostral deve conter polígonos."); return None
            ...
            if gdf.empty or not gdf.geom_type.isin(['Point', 'MultiPoint']).any():
                st.error("❌ O arquivo de pontos deve conter geometrias do tipo Ponto."); return None
            ...
            if not ((gdf['latitude'].between(-90, 90)) & (gdf['longitude'].between(-180, 180))).all():
                st.error("❌ Latitude/Longitude fora do intervalo esperado."); return None
            st.session_state.gdf_pontos = gdf
            st.success(f"✅ {len(gdf)} pontos carregados com sucesso!")
            return gdf
    except Exception as e:
        st.error("❌ Erro ao processar arquivo."); st.exception(e); return None

def salvar_no_streamlit_cloud():
    if st.session_state.get("gdf_poligono") is None:
        st.warning("⚠️ Defina a área amostral antes de salvar!"); return
    if st.session_state.get("densidade_plantas") is None or st.session_state.get("produtividade_media") is None:
        st.warning("⚠️ Parâmetros de densidade e produtividade não definidos!"); return
    ...
    if st.session_state.get("gdf_pontos") is not None and not st.session_state.gdf_pontos.empty:
        st.session_state.gdf_pontos.to_file(os.path.join(save_dir, "pontos_produtividade.gpkg"), driver="GPKG")
    ...
    with open(params_path, "w") as f: json.dump(parametros, f)
...
def exportar_dados():
    if st.session_state.gdf_poligono is None:
        st.warning("⚠️ Defina a área amostral antes de exportar!"); return

    tmp_dir = "/tmp/export_zip"; os.makedirs(tmp_dir, exist_ok=True)
    ...
    with open(zip_path, "rb") as f:
        st.download_button("💾 Exportar dados (ZIP)", data=f.read(),
                           file_name="dados_produtividade.zip", mime="application/zip")
...
def inserir_produtividade():
    ...
    with st.form("form_produtividade", clear_on_submit=False):
        ...
        for idx, _ in gdf.iterrows():
            col = cols[idx % 3]
            with col:
                st.number_input(f"Ponto {idx+1}", key=f"valor_pt_{idx}", min_value=0.0, step=0.01, format="%.2f")
...
with b2:
    ...
    if st.session_state.voice_mode:
        ...
        if audio is not None:
            ...
            with open(wav_path, "wb") as f: f.write(audio.getvalue())
            rec = sr.Recognizer()
            try:
...
with b3:
    st.markdown("**Parâmetros da área (opcional)**")
