...
try:
    ...
    try:
        ...
        print("Não foi possível instalar a biblioteca Fiona. Algumas funcionalidades serão limitadas.")
...
produtividade_input = FloatText(
    description='Produtividade média (sacas/ha):',
    value=0.0,
    style={'description_width': 'initial'}
)
...
def handle_densidade(b):
    ...
    with output:
        output.clear_output()
        print(f"✅ Densidade definida: {densidade_plantas} plantas/hectare")
...
def handle_produtividade(b):
    ...
    with output:
        output.clear_output()
        print(f"✅ Produtividade média definida: {produtividade_media} sacas/hectare")

btn_inserir_densidade.on_click(handle_densidade)
btn_inserir_produtividade.on_click(handle_produtividade)

# Botões principais
btn_adicionar_poligono = Button(description='▶️ Área amostral', button_style='primary')
btn_adicionar_poligono_2 = Button(description='▶️ Área total', button_style='primary')
btn_limpar_poligono = Button(description='🗑️ Limpar Área', button_style='danger', disabled=True)
btn_gerar_pontos_poligono = Button(description='🔢 Gerar pontos', button_style='info', disabled=True)
btn_inserir_manual = Button(description='✏️ Inserir pontos', button_style='info', disabled=True)
btn_inserir_voz = Button(description='🎤 Inserir por Voz', button_style='warning', disabled=True)
btn_inserir_prod = Button(description='📝 Inserir produtividade', button_style='success', disabled=True)
btn_salvar_pontos = Button(description='💾 Salvar pontos', button_style='success', disabled=True)
exportar_btn = Button(description='💾 Exportar dados', button_style='info', icon='save')

# Botões adicionais
btn_adicionar_pontos = Button(description='🔘 Adicionar pontos', button_style='info', disabled=True)
btn_gerar_auto = Button(description='🤖 Gerar automaticamente', button_style='primary')
btn_inserir_voz_gps = Button(description='🎤 Inserir por voz (GPS)', button_style='warning')
btn_inserir_manual_gps = Button(description='✏️ Inserir pontos (GPS)', button_style='warning')
btn_ativar_gps = ToggleButton(description='📍 Ativar GPS', value=False, button_style='success', icon='satellite')
btn_carregar_arquivo = Button(description='📂 Carregar arquivo', button_style='info')

# Widget para seleção de arquivo
file_chooser = FileChooser()
file_chooser.default_path = os.getcwd()
file_chooser.filter_pattern = '*.shp','.kml','.kmz','.gpkg'
file_chooser.title = 'Selecione polígono'
file_chooser.layout.display = 'none'

# Painel com opções adicionais para coleta
pontos_options = VBox([
    HTML("<h4>Método de Adição de Pontos:</h4>"),
    btn_gerar_auto,
    btn_inserir_voz_gps,
...
def toggle_pontos_options(b):
    ...
    with output:
        ...
        if pontos_options.layout.display == 'block':
            print("🔘 Modo de adição de pontos ativado")
...
def limpar_poligono(b):
    ...
    for layer in Map.layers:
        if layer.name == "Área de Interesse" or layer.name.startswith("Ponto "):
    ...
    with output:
        output.clear_output()
        print("Área removida. Desenhe novamente ou carregue um arquivo.")

btn_limpar_poligono.on_click(limpar_poligono)

# Desenhar polígono da área amostral
def handle_draw(target, action, geo_json):
    global gdf_poligono
    if geo_json['geometry']['type'] == 'Polygon':
        for layer in Map.layers:
            if layer.name == "Área amostral":
                Map.remove_layer(layer)
        try:
            ...
            Map.addLayer(feature, {'color': 'red', 'fillColor': 'red', 'fillOpacity': 0.2}, "Área de Interesse")
...
def handle_draw_total(target, action, geo_json):
    ...
    if geo_json['geometry']['type'] == 'Polygon':
        try:
            ...
            Map.addLayer(feature, {'color': 'blue', 'fillColor': 'blue', 'fillOpacity': 0.15}, "Área Total")
            with output:
                output.clear_output()
                print("✅ Área total definida!")
        except Exception as e:
            with output:
                print(f"Erro: {e}")
...
def verificar_areas_criadas():
    """Verifica se ambas as áreas (amostral e total) foram criadas e habilita os botões de geração de pontos"""
    if gdf_poligono is not None and gdf_poligono_total is not None:
        btn_gerar_pontos_poligono.disabled = False  # Habilita botão para área amostral
        with output:
            print("✅ Áreas amostral e total definidas!")
            print("Gere pontos da área amostral")
...
def handle_draw_unificado(target, action, geo_json):
    ...
    if modo_desenho == 'amostral':
        ...
        with output:
            clear_output()
            print("✅ Área amostral definida.")
        Map.add_gdf(gdf_poligono, layer_name="Área Amostral", style={"color": "blue"})
    elif modo_desenho == 'total':
        gdf_poligono_total = gdf_temp
        with output:
            clear_output()
            print("✅ Área total definida.")
        Map.add_gdf(gdf_poligono_total, layer_name="Área Total", style={"color": "green"})

    # Verifica se ambas as áreas foram criadas
    verificar_areas_criadas()

# Conecte a função ao mapa
...
def carregar_arquivo(b):
    ...
    try:
        ...
        with output:
            print(f"❌ Erro ao abrir seletor de arquivos: {str(e)}")

btn_carregar_arquivo.on_click(carregar_arquivo)

def ler_arquivo_geografico(file_path):
    """Lê arquivos geográficos nos formatos suportados"""
    if file_path.endswith('.gpkg'):
        ...
        raise ValueError("❌ Formato de arquivo não suportado! Formatos aceitos: .gpkg, .shp, .kml, .kmz")

def processar_poligono_no_mapa(gdf, nome_layer="Área de Interesse"):
    """Valida e plota polígono no mapa"""
    global gdf_poligono

    if not any(gdf.geometry.type.isin(['Polygon', 'MultiPolygon'])):
        raise ValueError("⚠️ O arquivo não contém polígonos!")
...
def handle_file_selection(change):
    ...
    try:
        ...
        with output:
            ...
            print(f"Polígonos: {len(gdf_poligono)}")
...
def handle_upload(change):
    ...
    try:
        ...
        with output:
            output.clear_output()
            print(f"⏳ Processando arquivo enviado: {filename}")
...
file_chooser = FileChooser(
    ...
    title='Selecione o arquivo com o polígono'
)
file_chooser.layout.display = 'none'
...
def gerar_pontos_automaticos(b):
    ...
    if gdf_poligono is None:
        with output:
            print("⚠️ Defina a área primeiro!")
        return
    ...
    for idx, ponto in gdf_pontos.iterrows():
        marker = geemap.geojson_to_ee({
            ...
            "properties": {"status": "Não coletado"}
        })
        Map.add_layer(marker, {'color': 'orange'}, f"Ponto {idx+1}")

    with output:
        output.clear_output()
        print(f"✅ {len(gdf_pontos)} pontos gerados automaticamente!")
        print(f"Área total: {area_ha:.2f} hectares")
...
# -----------------------------
# FUNÇÕES DE ADIÇÃO MANUAL DE PONTOS
# -----------------------------
def adicionar_ponto_manual_gps(b):
    if not btn_ativar_gps.value:
        with output:
            print("⚠️ Ative o GPS primeiro!")
        return

    # Simulação de coleta de GPS - na prática integraria com biblioteca de geolocalização
...
def adicionar_ponto_manual(b):
    # Cria interface para inserção manual de coordenadas
...
# Função para abrir diálogo de produtividade
def abrir_dialogo_produtividade(idx):
    ...
    def salvar_dados(b):
        ...
        with output:
            output.clear_output()
            print(f"✅ Dados salvos no Ponto {idx+1}")
...
# Atualize a função adicionar_ponto para manter consistência
def adicionar_ponto(lat, lon, metodo):
    global gdf_pontos
    ...
    # Adiciona ao mapa (cor laranja para pontos não coletados)
    marker = geemap.geojson_to_ee({
        "type": "Feature",
        "geometry": mapping(ponto),
        "properties": {"status": "Não coletado"}
    ...
    return len(gdf_pontos) - 1  # Retorna o índice do ponto adicionado

btn_adicionar_pontos.on_click(toggle_pontos_options)

# -----------------------------
# FUNÇÕES DE INSERÇÃO POR VOZ
# -----------------------------
def capturar_por_voz_com_gps(b):
    ...
    # Simulação de GPS - substituir por coordenadas reais
    lat, lon = -15.123, -55.456

    with output:
        output.clear_output()
        print("🎤 Aguarde... ouvindo valor de produtividade")
    ...
    try:
        ...
        # Atualiza o último ponto adicionado
        ...
        with output:
            print(f"✅ Valor '{valor}' inserido no Ponto {len(gdf_pontos)}")

    except Exception as e:
        with output:
            print(f"❌ Erro ao reconhecer voz: {str(e)}")

btn_inserir_voz_gps.on_click(capturar_por_voz_com_gps)
btn_inserir_voz.on_click(capturar_por_voz_com_gps)

# -----------------------------
# FUNÇÕES DE INSERÇÃO DE PRODUTIVIDADE
# -----------------------------
def inserir_produtividade(b):
    if gdf_pontos is None or gdf_pontos.empty:
        with output:
            print("⚠️ Nenhum ponto disponível para inserção de dados.")
        return

    # Cria interface para edição dos pontos
    pontos_widgets = []
    ...
    def salvar_produtividade(btn):
        ...
        for idx, row in gdf_pontos.iterrows():
            ...
            marker = geemap.geojson_to_ee({
                ...
                "properties": {"status": "Coletado" if row['coletado'] else "Não coletado"}
            })
            Map.add_layer(marker, {'color': color}, f"Ponto {idx+1}")

        with output:
            output.clear_output()
            print("✅ Dados de produtividade salvos com sucesso!")

    salvar_btn = Button(description='💾 Salvar Todos', button_style='success')
...
def salvar_pontos(b):
    ...
    if gdf_pontos is None or gdf_pontos.empty:
        with output:
            print("⚠️ Nenhum ponto para salvar!")
        return
    ...
    with output:
        print("✅ Dados dos pontos preparados para exportação!")

btn_salvar_pontos.on_click(salvar_pontos)

# exportar dados
def exportar_dados(b):
    """Exporta os dados no formato compatível com o código de processamento"""
    global densidade_plantas, produtividade_media

    output.clear_output()

    # Exportar parâmetros como JSON
    parametros = {
    ...
    with output:
        print(f"✅ Parâmetros da área exportados: {parametros_path}")
        print(f"- Densidade: {densidade_plantas} plantas/ha")
        print(f"- Produtividade média: {produtividade_media} sacas/ha")
    ...
    if gdf_poligono is not None:
        ...
        if gdf_pontos is not None and not gdf_pontos.empty:
            ...
            with output:
                print(f"✅ Arquivo do polígono exportado: {poligono_path}")
                print(f"✅ Arquivo de pontos exportado: {pontos_path}")
                print(f"Total de pontos exportados: {len(gdf_pontos)}")
        else:
            with output:
                print(f"✅ Arquivo do polígono exportado: {poligono_path}")
                print("ℹ️ Nenhum ponto de produtividade foi adicionado para exportação")
    else:
        with output:
            print("⚠️ Nenhuma área desenhada para exportar")
    ...
    if gdf_poligono_total is not None:
        ...
        with output:
            print(f"✅ Polígono total exportado: {poligono_total_path}")
    else:
        with output:
            print("⚠️ Nenhum polígono total disponível para exportar.")
...
with output:
    ...
    print("1. Desenhe uma área ou carregue um arquivo")
...
if gdf_poligono is not None:
    ...
    poligono_path = '/content/area_poligono.gpkg'
    gdf_poligono.to_file(poligono_path, driver='GPKG')
    print(f"\n✅ Arquivo do polígono exportado: {poligono_path}")
    ...
    if gdf_pontos is not None and not gdf_pontos.empty:
        ...
        print("\nℹ️ Nenhum ponto de produtividade foi adicionado para exportação")
else:
    print("\n⚠️ Nenhuma área definida para exportação")
...
# Cálculo da área total em hectares
area_ha = gdf_poligono.geometry.area.sum() / 10000
print(f"\nÁrea total calculada a partir do polígono carregado: {area_ha:.2f} hectares")

# Mostrar conteúdo do GeoDataFrame de pontos
print("\nDados do GeoDataFrame de pontos:")
display(gdf_pontos.head())

# Mostrar a contagem de feições e os nomes das colunas
print(f"\nPolígono contém {len(gdf_poligono)} feições e as colunas: {list(gdf_poligono.columns)}")
print(f"Pontos contêm {len(gdf_pontos)} feições e as colunas: {list(gdf_pontos.columns)}")
...
if datas:
    print(f"{len(datas)} imagens válidas:")
    for data in sorted(datas):
        print(f"- {data}")
else:
    print("\nNenhuma imagem válida encontrada com dados na área especificada.")

# Definir os índices que serão extraídos
...
# Separar versão sem geometria (em memória, sem salvar)
...
# 11. Predição para todos os pontos usando o melhor modelo
print("\n=== Predição para todos os pontos ===")

# Preparar os dados completos
X_full = X  # Já carregamos todos os dados anteriormente
y_full = y   # Todos os valores reais

# Verificar se precisamos escalonar os dados para o melhor modelo
if melhor_modelo_nome in modelos_escalonados:
    # Usar o mesmo scaler da melhor execução
    scaler = StandardScaler()
    X_full_scaled = scaler.fit_transform(X_full)  # Fit apenas para evitar warning, já que estamos usando todos os dados
    X_for_prediction = X_full_scaled
else:
    X_for_prediction = X_full

# Fazer as predições para todos os pontos
...
df_full_comparison = pd.DataFrame({
    ...
    'Resíduo': y_full - y_full_pred
})

# Calcular erro relativo
df_full_comparison['Erro_Relativo'] = (df_full_comparison['Resíduo'] / df_full_comparison['Produtividade_Real']) * 100

# Ordenar por produtividade real para melhor visualização
...
# 12. Avaliação estatística com TODOS os dados
...
pes_por_amostra = 5  # 5 pés por amostra

# Cálculo do fator de conversão
area_por_ponto_ha = (1/densidade_pes_ha) * pes_por_amostra  # Área que cada ponto representa em hectares
print(f"Cada ponto amostral representa: {area_por_ponto_ha:.6f} ha")

# Criar dataframe com valores reais e preditos (usando o código complementar anterior)
...
# 1. Cálculo com base nos valores REAIS de todos os 20 pontos
...
# 2. Cálculo com base nos valores PREDITOS de todos os 20 pontos
...
# 3. Comparação com a produtividade média conhecida
print("\n=== COMPARAÇÃO DE PRODUTIVIDADES (20 PONTOS) ===")
print(f"Produtividade média conhecida da área: {sacas_ha} sacas/ha")
...
print(f"- Diferença: {produtividade_real_sacas_ha - sacas_ha:.2f} sacas/ha")

print(f"\nBaseado nas previsões do modelo {melhor_modelo_nome} para 20 pontos:")
...
print(f"- Diferença: {produtividade_predita_sacas_ha - sacas_ha:.2f} sacas/ha")

# 4. Cálculo da produção total estimada
...
print(f"Produção total REAL para {area_total_ha} ha:")
print(f"- {producao_total_real_kg:.2f} kg")
print(f"- {producao_total_real_sacas:.2f} sacas")

print(f"\nProdução total PREDITA para {area_total_ha} ha:")
...
print("\n=== DISCREPÂNCIAS ===")
print(f"Discrepância das amostras reais: {discrepancia_percentual_real:.2f}%")
print(f"Discrepância do modelo {melhor_modelo_nome}: {discrepancia_percentual_predita:.2f}%")
...
# 1. Dados já existentes: gdf_resultado (pontos com índices + produtividade) e gdf_poligono
...
# Criar coluna de identificação dos pontos
gdf_resultado['Ponto'] = gdf_resultado.index + 1

# Carregar as predições de produtividade
...
# gdf_poligono: contém o polígono da área

# Criar novo GeoDataFrame apenas com o necessário
...
res = 5  # resolução em metros
xi = np.arange(xmin, xmax, res)
yi = np.arange(ymin, ymax, res)
xi_grid, yi_grid = np.meshgrid(xi, yi)

# Função para aplicar máscara do polígono
...
# Adiciona krigagem se disponível
...
# Verifica se método está disponível
if METHOD not in METHODS:
    raise ValueError(f"Método '{METHOD}' não disponível. Use um dos: {', '.join(METHODS.keys())}")

# Aplica interpolação
print(f"Usando método: {METHODS[METHOD]['description']}")
...
# Atualização para exibir lat/lon
...
# Texto inferior com SRC e crédito
ax.text(0.5, -0.12, "SCR: WGS84 / 24S | July 27, 2025 | Crédito: Mário Bittencourt",
...
# --- Função para processar coleção com filtro de nuvem adaptado ---
def processar_colecao(start_date, end_date, roi):
    # Define limite de nuvens conforme o período
    ...
    colecao_base = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED') \
        ...
        .map(lambda img: img.addBands([
            # Cálculo de todas as bandas necessárias
    ...
    # Obter lista de datas únicas
    datas_unicas = colecao_base.aggregate_array('data').distinct()

    # Para cada data única, pegar apenas a primeira imagem correspondente
...
# --- [MANTIDAS TODAS AS FUNÇÕES AUXILIARES ORIGINAIS] ---
def extrair_estatisticas_ponto_imagem(imagem, feature_ponto, nomes_indices):
...
# --- 1. Carregar área de interesse e definir ROI ---
...
print(f"Produtividade média prevista: {dados_predicao['produtividade_sc/ha'].mean():.2f} sacas/ha")
...
def verificar_colecao_imagens(colecao, nome_colecao):
    print(f"\n{'='*60}")
    print(f"VERIFICAÇÃO DE IMAGENS - {nome_colecao.upper()}".center(60))
    print(f"{'='*60}\n")

    total = colecao.size().getInfo()
    print(f"Total de imagens na coleção {nome_colecao}: {total}")
...
verificar_colecao_imagens(colecao_pred, "Predição")
...
# 3. Função para processar a coleção de imagens
def processar_colecao(start_date, end_date, roi):
    # Define limite de nuvens conforme o período
    if start_date == '2023-08-01' and end_date == '2024-05-31':
        limite_nuvens = 5
...
# 4. Carregar área de interesse
area_interesse = gpd.read_file('/content/area_poligono.gpkg')
roi = geemap.geopandas_to_ee(area_interesse)

# 5. Processar coleções de imagens
colecao_treino = processar_colecao(*periodo_treinamento, roi)
colecao_pred = processar_colecao(*periodo_predicao, roi)

# 6. Função para calcular médias dos índices na área
...
for i, indice in enumerate(bandas, 1):
    ...
    plt.title(f'Evolução do {indice}')
...
# === 1. LER ARQUIVOS ===
# Caminho dos dados preditos da safra futura (do código 1)
csv_predito = '/content/predicao_sem_datas.csv'
gdf_area = gpd.read_file('/content/area_poligono.gpkg').to_crs("EPSG:4326")

# Lê o CSV com os dados previstos e extrai coordenadas da geometria
...
# === 2. CRIAR GRID REGULAR PARA INTERPOLAÇÃO ===
...
# === 4. APLICAR MÁSCARA DO POLÍGONO DA ÁREA ===
def apply_mask(zi, xi_grid, yi_grid, gdf_area):
    poly_path = Path(gdf_area.geometry.unary_union.exterior.coords)
...
# Rodapé
ax.text(0.5, -0.12, "SCR: WGS84 / 24S | Safra do Café",
        transform=ax.transAxes, ha='center', va='center', fontsize=9)
...
# Layout dos subplots (3 colunas, número de linhas ajustado ao total de índices)
...
for i, indice in enumerate(indices):
    ...
    ax.set_title(f"{indice}\nDistância Euclidiana (z): {distancia_individual:.4f}")
    ax.set_xlabel("Valor")
...
plt.tight_layout()
plt.suptitle("Distribuições dos Índices: Safra Treinamento vs Safra Predição", fontsize=16, y=1.02)
plt.show()

# Cálculo da distância global
...
def interpret_zscore_distance(distancia):
    ...
    print(f"{'0.5 – 1.5':<15} Small differences — model likely still valid")
    print(f"{'1.5 – 2.5':<15} Moderate differences — assess model sensitivity")
    print(f"{'> 2.5':<15} Significant differences — consider new calibration")

    print("\nAutomatic summary based on calculated distance:")
    if distancia < 0.5:
        print(">> Spectrally almost identical seasons.")
    elif distancia < 1.5:
        print(">> Small differences — model likely still valid.")
    elif distancia < 2.5:
        print(">> Moderate differences — assess model sensitivity.")
    else:
        print(">> Significant differences — new calibration or model may be needed.")

# Executa interpretação
interpret_zscore_distance(distancia_total)

# GERAR PONTOS ÁREA TOTAL
...
### GERAR PONTOS NA ÁREA TOTAL

# 1. Carregar polígono e calcular área (ha)
...
print(f"Área total: {area_ha_total:.2f} ha | Pontos a gerar: {num_pontos_total}")
...
if 'kg' not in df_treino.columns:
    raise KeyError("Coluna 'kg' não encontrada nos dados de treino!")
...
# 6. Carregar dados de predição e validar colunas
...
if colunas_faltando:
    raise ValueError(f"Colunas faltando nos dados de predição: {colunas_faltando}")
...
print(f"Produtividade média prevista: {media_predita:.2f} sc/ha")

# 8. Salvar resultados
df_pred.to_csv('/content/predicao_safra_2025.csv', index=False)
print("Predição salva em: /content/predicao_safra_2025.csv")
